...
(function($) {
    ...
    $.fn.Tabledit = function(options) {
                ...
                identifier: function() {
                  if(settings.method!="two"){
                    ...
                    $td.each(function() {
                        ...
                        $(this).parent('tr').attr(settings.rowIdentifier, 'row'+$(this).text());

                        $(this).parent('tr').attr('data-id', $(this).text());
                    });
                  }
                  else if(settings.method=="two"){
                    let valueIdentifier = "", nameIdentifier = settings.tableIdentifier;

                    valueIdentifier = $table.attr(nameIdentifier);

                    $table.find('tr').each(function(){
                      $(this).attr(settings.rowIdentifier, 'row'+valueIdentifier);

                      $(this).attr('data-id', valueIdentifier);
                    });
                  }
                },
                editable: function() {
                    ...
                    if(settings.method=="two"){
                      $td = $table.find('tbody td[data-state=enabled]');

                      $td.each(function(number){
                          let cantTD = $td.length;
                          ...
                          let dataField = $(this)[0].dataset.field;

                          var input = '<input class="tabledit-input ' + settings.inputClass + '" type="text" name="' + dataField + '" value="' + $(this).text() + '" style="display: none;" disabled>';

                          // Add elements and class "view" to table cell.
                          let hasClass = $(this).hasClass('tabledit-view-mode').toString();
                          ...
                          if(hasClass == "false")
                          {
                            ...
                            // {
                              ...
                              //editt
                              // $(this).attr('data-field', settings.columns.editable[i][1]);
                            // }
                          }
                        });
                    }
                      ...
                      for (var i = 0; i < settings.columns.editable.length; i++) {
                          $td = $table.find('tbody td:nth-child(' + (parseInt(settings.columns.editable[i][0]) + 1) + ')');
                          ...
                          $td.each(function(number) {
                              ...
                              if (typeof settings.columns.editable[i][2] !== 'undefined') {
                                ...
                                var selectInit = '<select class="tabledit-input ' + settings.inputClass + '" name="' + settings.columns.editable[i][1] + '" style="display: none;" disabled>';
                                ...
                                $.each(jQuery.parseJSON(settings.columns.editable[i][2]), function(index, value) {
                                  ...
                                  if(index=="all"){
                                    if(text === value){
                                      inputFirst += '<option value="' + index + '" selected>' + value + '</option>';
                                    }
                                    else{
                                      inputFirst += '<option value="' + index + '">' + value + '</option>';
                                    }
                                  }
                                  else{
                                    if(text === value){
                                ...
                                input = selectInit + inputFirst + input;
        ...
        function ajax(action){
            let serialize = $table.find('.tabledit-input').serialize();

            if(!serialize){
                return false;
            }

            if(settings.method=="three"){
              let nameInput = serialize.split('&'), nameInput_, name, value;

              nameInput_ = nameInput[1].split('=');

              name = nameInput_[0];

              value = nameInput_[1];

              serialize = serialize.replace("&"+nameInput[1],"");

              serialize = serialize+"&name="+name+"&value="+value+"&view="+$table.attr('data-view');
            }

            if(settings.method=="two"){
              let valueIdentifier = "", nameIdentifier = settings.tableIdentifier, valueName="", initSerialize = serialize, findSelectInput = $table.closest('.weeks').siblings('.selectInput');

              valueIdentifier = $table.attr(nameIdentifier);

              valueName = serialize.split('=');

              serialize += "name="+valueName[0];

              serialize += '&'+"value="+valueName[1];

              serialize += '&'+"nameId="+nameIdentifier;

              serialize += '&'+"valueId="+valueIdentifier;

              serialize = serialize.replace(initSerialize, "");

              if(findSelectInput.length>=1){
                let month = findSelectInput.find('.selectMonth').val(), year = findSelectInput.find('.inputYear').val();

                let result = "&month="+month+"&year="+year;
